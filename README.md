...
- **默认 HTML 页面截图**：保留当前主题排版，可用「字体缩放（%）」调节内容密度。
- **手机自适应截图**：`1080 × 1440` 的 3:4 排版，可用「正文字号（px）」控制每张图的内容密度。

两种模式均提供宽度、高度、内边距等完整参数，参数面板默认折叠。点击「★ 设为默认」可将当前参数保存为该模式的专属默认值；「↺ 出厂值」还原系统初始参数。预览图区可折叠，方便直接操作下方的文案区。

### LLM 多平台配置

点工具栏 `⚙️ LLM`，支持多套配置同时保存（每套含接口地址 + 模型 + API Key）：

- **快捷预设**：硅基流动 / DeepSeek / OpenRouter / Groq / Ollama / OpenAI，选择后自动填充地址和模型名
- **多配置切换**：「保存并使用」创建新配置，右侧列表一键切换，API Key 加密保存在本地，无需重复输入
- 本地 Ollama 无需 API Key，填 `http://localhost:11434/v1` 即可
...
- 🎛️ **小红书双模式参数独立控制**：默认截图模式提供「字体缩放（%）」，自适应模式提供「正文字号（px）」，两种模式参数互不干扰
