...
/**
 * 下载远程图片并内联为 base64 data URI（用于复制到知乎/微信等拒绝外链的编辑器）。
 * 本地图片在 renderMarkdown 阶段已转 data URI；这里只处理 http(s) 图片。
 * 失败（网络/超时/超限）的图片保留原 URL，不阻塞复制。
 * @param {string} bodyHtml
 * @param {{timeoutMs?:number, maxBytes?:number}} [opts]
 * @returns {Promise<string>}
 */
async function inlineRemoteImages(bodyHtml, opts = {}) {
  const timeoutMs = opts.timeoutMs || 10000;
  const maxBytes = opts.maxBytes || 8 * 1024 * 1024;
  const urls = [...new Set(
    [...bodyHtml.matchAll(/<img[^>]*\ssrc="(https?:\/\/[^"]+)"/gi)].map(m => m[1])
  )];
  if (!urls.length) return bodyHtml;

  const results = {};
  await Promise.all(urls.map(async (url) => {
    try {
      results[url] = await downloadImageToDataUrl(url, timeoutMs, maxBytes);
    } catch (_) { /* 保留原 URL */ }
  }));

  if (!Object.keys(results).length) return bodyHtml;
  return bodyHtml.replace(
    /<img([^>]*)\ssrc="(https?:\/\/[^"]+)"/gi,
    (m, attrs, url) => (results[url] ? `<img${attrs} src="${results[url]}"` : m)
  );
}

/** 下载单个图片并转 data URL（http/https，带超时与大小上限） */
function downloadImageToDataUrl(url, timeoutMs, maxBytes) {
  return new Promise((resolve, reject) => {
    ...
    const req = client.get(url, { timeout: timeoutMs, headers: { 'User-Agent': 'Mozilla/5.0' } }, (res) => {
      if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
        ...
        client.get(loc, { timeout: timeoutMs, headers: { 'User-Agent': 'Mozilla/5.0' } }, (res2) => {
          if (res2.statusCode !== 200) { fail(new Error('HTTP ' + res2.statusCode)); return; }
          res2.on('data', (c) => {
            total += c.length;
            if (total > maxBytes) { fail(new Error('too large')); req.destroy(); return; }
            chunks.push(c);
          });
      ...
      res.on('end', () => {
        ...
        const mime = guessImageMime(buf, url);
...
/** 根据魔数或扩展名猜测图片 MIME */
function guessImageMime(buf, url) {
  if (buf.length > 3 && buf[0] === 0xFF && buf[1] === 0xD8 && buf[2] === 0xFF) return 'image/jpeg';
  if (buf.length > 7 && buf[0] === 0x89 && buf[1] === 0x50 && buf[2] === 0x4E && buf[3] === 0x47) return 'image/png';
  if (buf.length > 5 && buf[0] === 0x47 && buf[1] === 0x49 && buf[2] === 0x46) return 'image/gif';
  if (buf.length > 11 && buf[0] === 0x52 && buf[1] === 0x49 && buf[2] === 0x46 && buf[3] === 0x46 && buf[8] === 0x57 && buf[9] === 0x45 && buf[10] === 0x42 && buf[11] === 0x50) return 'image/webp';
...
function buildFullHtml(bodyHtml, templatePath) {
  ...
  // 公式转内联 SVG（mdnice 格式）：导出的 HTML 在任何浏览器/网页里公式都能显示，
  // 不依赖 KaTeX 字体与 CSS。曾用 KaTeX HTML + 无字体 CSS，公式在外部环境会消失。
  convertMathToSvgDataUri($);

  // 加入 KaTeX CSS（含 base64 内联字体，避免外部字体路径失效）
  $('head').append(`<style>${getKatexCssBase64()}</style>`);
...
function getMjxState() {
  ...
  const { BoldsymbolConfiguration } = require('mathjax-full/js/input/tex/boldsymbol/BoldsymbolConfiguration.js');

  // AllPackages 不含 boldsymbol 宏包：\boldsymbol/\pmb 可用，但 \bm 会被当作
  // 未知命令，导致公式里出现红色 "bm" 文字。手动补上宏包 + \bm 宏。
  // 注意：macros 里 '\\boldsymbol' 是无参数宏（\bm 展开为 \boldsymbol），
  // 不能写成 ['boldsymbol', 1]——那会把 'boldsymbol' 当字面文字渲染成斜体 "boldsymbol"。
  const packages = AllPackages.concat(['boldsymbol']);
...
function trimImageBlankParagraphs(html) {
  // 需求：图片（含 figure）上下不要有空行，尤其是移动端阅读时。
  // 原实现只压缩"连续多个"空段落，单个 <p></p>（markdown 空行渲染结果）会残留，
  // 导致粘贴到微信/知乎后图片上下仍有多余空行。
  // 这里改为：图片/figure 前或后的空段落一律删除（一个不留）。
  let result = html.replace(
    /(?:<p[^>]*>(?:&nbsp;|\s|<br\s*\/?>)*<\/p>\s*)+(?=\s*<(?:figure|img))/gi,
    ''
  );
  result = result.replace(
    /(?<=<\/(?:figure|img[^>]*?)>\s*)(?:\s*<p[^>]*>(?:&nbsp;|\s|<br\s*\/?>)*<\/p>)+/gi,
    ''
  );
  // 裸 <img>（自闭合，无 </img>）：删掉紧跟其后的空段落
  result = result.replace(
    /(?<=<img[^>]*>\s*)(?:\s*<p[^>]*>(?:&nbsp;|\s|<br\s*\/?>)*<\/p>)+/gi,
    ''
  );
  // 兜底：非图片区域的连续空段落压缩为一个（保守，不过度删）
  result = result.replace(
    /(<p[^>]*>(?:&nbsp;|\s|<br\s*\/?>)*<\/p>\s*){3,}/gi,
    '<p><br></p>'
  );
