...
function juiceWithNbsp(html, juiceOpts) {
  ...
  return result.replace(NBSP_RE, '&nbsp;').replace(BR_RE, '<br>');
}
...
function enhanceCodeBlocks($) {
  ...
  $('pre').each((_, elem) => {
    ...
    pre.replaceWith(
      `<pre class="mac-code" style="font-size:90%;overflow-x:auto;border-radius:8px;padding:0;line-height:1.5;margin:10px 8px;background-color:#f6f8fa;border:1px solid #eaedf0;"><span class="mac-dots" style="display:block;margin:12px 16px 0;">${svgDots}</span>` +
      `<code class="hljs ${language}" style="display:block;padding:0.5em 1em 1em;overflow-x:auto;text-indent:0;color:inherit;background:none;white-space:pre;margin:0;">${highlighted}</code></pre>`
    );
  });
}
...
function buildWechatCopyHtml(bodyHtml, templatePath, theme) {
  ...
  // 2. 注入 KaTeX CSS，字体转 base64（避免路径问题，模块级缓存）
  $('head').append(`<style>${getKatexCssBase64()}</style>`);

  // 3. juice 内联 CSS
  const finalHtml = juiceWithNbsp($.html());
