 ...
 */
function buildXhsAdaptiveRenderHtml(bodyHtml, baseDir, theme) {
  const katexCss = getKatexCssBase64();
  const $ = cheerio.load(bodyHtml, { decodeEntities: false });
  $('img').each((_, el) => {
    const src = $(el).attr('src') || '';
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .article-wrapper {
      width: 100%; padding: 56px 44px; background: ${bgColor}; color: #1a1a1a;
      font-family: 'PingFang SC', 'Hiragino Sans GB', 'Microsoft YaHei UI',
        'Microsoft YaHei', system-ui, -apple-system, sans-serif;
      font-size: 40px; line-height: 1.78; -webkit-font-smoothing: antialiased;
    }
    .article-wrapper p { margin: 1.1em 0; }
    .article-wrapper img { max-width: 100%; display: block; margin: 1.4em auto; border-radius: 16px; }
    .article-wrapper strong { font-weight: 700; color: #ff2442; }
    .article-wrapper em { color: #444; }
    .article-wrapper a { color: #ff2442; text-decoration: none; }
    .article-wrapper h1 {
      margin: .2em 0 1.2em; padding: 28px 32px; border-radius: 20px;
      background: #fff0f3; color: #1a1a1a; text-align: center;
      font-size: 56px; font-weight: 800; line-height: 1.35;
    }
    .article-wrapper h2 {
      margin: 1.6em 0 .8em; padding: 18px 24px 18px 28px;
      border-left: 8px solid #ff2442; border-radius: 0 14px 14px 0;
      background: #fff8f9; color: #1a1a1a; font-size: 48px; line-height: 1.4;
    }
    .article-wrapper h3 {
      margin: 1.4em 0 .6em; padding-left: 20px; border-left: 6px solid #ffb3c1;
      color: #ff2442; font-size: 42px; line-height: 1.45;
    }
    .article-wrapper h4, .article-wrapper h5, .article-wrapper h6 {
      margin: 1.2em 0 .5em; color: #333; font-size: 38px;
    }
    .article-wrapper blockquote {
      margin: 1.4em 0; padding: 28px 32px; border: 2px solid #ffd6dc;
      border-radius: 16px; background: #fff8f9; color: #555;
    }
    .article-wrapper blockquote p { margin: 0; }
    .article-wrapper table { width: 100%; margin: 1.4em 0; border-collapse: collapse; font-size: 34px; }
    .article-wrapper td, .article-wrapper th { padding: 16px 20px; border: 2px solid #ffd6dc; }
    .article-wrapper th { background: #ffe4e8; color: #cc1f3c; font-weight: 700; }
    ...
    .article-wrapper figcaption { margin-top: 12px; color: #999; font-size: 30px; text-align: center; }
    .article-wrapper code:not([class]) {
      padding: 4px 12px; border: 1px solid #ffb3c1; border-radius: 8px;
      background: #fff0f2; color: #cc1f3c; font-size: 34px;
    }
    .article-wrapper pre.mac-code {
      margin: 1.2em 0; overflow-x: auto; border: none; border-radius: 16px; background: #1e1e2e;
    }
    .article-wrapper pre.mac-code code.hljs { display: block; padding: 28px 32px; font-size: 30px; line-height: 1.65; }
    .article-wrapper hr { height: 4px; margin: 2em 0; border: none; background: #ffb3c1; }
    .article-wrapper .math-block { margin: 1.4em 0; padding: 12px 0; overflow-x: auto; text-align: center; }
    .article-wrapper .math-inline { display: inline; }
...
function buildXhsRenderHtmlByMode(bodyHtml, baseDir, theme, mode = 'classic') {
  return mode === 'adaptive'
    ? buildXhsAdaptiveRenderHtml(bodyHtml, baseDir, theme)
    : buildXhsRenderHtml(bodyHtml, baseDir, theme);
}
