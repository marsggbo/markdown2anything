...
// 扩展宿主里 process.execPath 是 VS Code 的 Electron 二进制，用它跑 Node worker
// 必须带 ELECTRON_RUN_AS_NODE=1，否则二进制不执行脚本（同 extension.js 的 NODE_EXEC_ENV）
...
async function getCookies(platform, storage) {
  try { return JSON.parse((await storage.get(STORAGE_KEYS[platform])) || '[]'); }
  catch (_) { return []; }
}
async function setCookies(platform, storage, cookiesArr) {
  await storage.set(STORAGE_KEYS[platform], JSON.stringify(cookiesArr || []));
}
async function clearCookies(platform, storage) {
  await storage.set(STORAGE_KEYS[platform], '');
}
...
function login(platform, { extensionPath, storage, onProgress, onNeedInstall, onChild }) {
  return new Promise((resolve, reject) => {
    ...
    const proc = spawn(process.execPath, [workerPath(extensionPath), 'login', platform, cookieOut], { env: NODE_EXEC_ENV });
    ...
    proc.on('close', async (code) => {
      ...
      try {
        ...
        await setCookies(platform, storage, cookies);
        resolve({ cookies });
...
function runWorker(cmd, platform, jobFile, extensionPath, { onProgress, onStep, onNeedInstall, onChild, resolve, reject }) {
  const proc = spawn(process.execPath, [workerPath(extensionPath), cmd, platform, jobFile], { env: NODE_EXEC_ENV });
