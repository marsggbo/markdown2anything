...
async function uploadImage(dataUrl, cookieStr) {
  ...
  // Step 2b: 无 upload_token，图片已存在于知乎图床

  // ★ 关键修复：优先用知乎接口返回的真实 CDN URL，而不是硬编码 pic4。
  // 之前硬编码 https://pic4.zhimg.com/80/v2-${hash}${ext}，域名/路径可能与知乎
  // 实际分配的不一致 → 粘贴到知乎编辑器时图片「加载失败」。
  // 真实 URL 必须通过「轮询」拿：OSS 上传后知乎后台异步处理图片，
  // 需要 GET /images/{image_id} 等 status=success 才返回 src（真实 CDN 地址）。
  // 之前少了这一步，只能硬编码 pic4 / 猜 upload_file.url → 图片传上去了但
  // URL 不对，粘贴进知乎编辑器就「加载失败」，图片看起来"不知去向"。
  const imageId = upload_file && upload_file.image_id;
  if (imageId) {
    try {
      const pollUrl = await pollImageStatus(imageId, cookieStr);
      if (pollUrl) return pollUrl;
    } catch (err) {
      // 轮询失败不阻塞，回退到猜的 URL
      console.error(`知乎图片轮询失败: ${err.message}`);
    }
  }

  // 回退：用 hash 构造（老逻辑，仅在接口未返回 url 时兜底）
  return `https://pic4.zhimg.com/80/v2-${hash}${ext}`;
}

/**
 * 轮询知乎图片处理状态，拿到真实 CDN URL。
 * 参考 zhihu-cli：GET /images/{image_id}，等 status=success，返回 data.src。
 * @param {string} imageId
 * @param {string} cookieStr
 * @returns {Promise<string|null>}
 */
async function pollImageStatus(imageId, cookieStr) {
  const xsrf = getCookieValue(cookieStr, '_xsrf');
 ...
 */
async function uploadImagesInHtml(htmlContent, cookieStr, onProgress, cache) {
  ...
  // 去重缓存：同一张图（按内容 md5）只上传一次，后续复用已上传的 CDN URL。
  // cache 是可选的 Map / 对象，由调用方（extension 层）传入并持久化。
  ...
  for (const el of imgs) {
    const src = $(el).attr('src');
    try {
      // 算 md5：data URI 内容的 hash
      const m = src.match(/^data:[^;]+;base64,(.+)$/);
      const buf = m ? Buffer.from(m[1], 'base64') : Buffer.from(src);
      const hash = md5(buf);
      if (cacheMap[hash]) {
        // 命中缓存，直接复用，不重复上传
        $(el).attr('src', cacheMap[hash]);
        reused++;
      } else {
        const cdnUrl = await uploadImage(src, cookieStr);
        $(el).attr('src', cdnUrl);
        cacheMap[hash] = cdnUrl;   // 上传成功后写入缓存
      }
    } catch (err) {
  ...
  return { html: $('#root').html() || htmlContent, total: imgs.length, failed: errors.length, errors, reused };
}
