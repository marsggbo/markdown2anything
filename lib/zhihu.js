...
async function uploadImage(dataUrl, cookieStr) {
  ...
  // 真实 URL 在 upload_file.url（首次上传）或 prefetchData.url（已存在）里。
  const realUrl =
    (upload_file && upload_file.url) ||     // 上传后知乎返回的最终地址
    prefetchData.url ||                      // 顶层可能也有
    (upload_file && upload_file.object_key && upload_file.object_key.startsWith('http')
      ? upload_file.object_key : null);
